...
Scans .agent/skills/ directory to build catalog at runtime.
...
CATEGORY_GUIDES = {
    ...
    "config": {
        ...
        "workflow": [
            ("Global", "Set user prefs in `~/.agent/.ck.json`"),
            ("Local", "Override per-project in `./.agent/.ck.json`"),
    ...
    "notifications": {
        ...
        "workflow": [
            ("1. Set env vars", "Add `DISCORD_WEBHOOK_URL` or `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID` to `~/.agent/.env`"),
            ("2. Add hook", "Add Stop hook to `.agent/settings.json` (see below)"),
            ("3. Test", "`echo '{\"hook_event_name\":\"Stop\"}' | node .agent/hooks/notifications/notify.cjs`"),
        ],
        "tip": """Add to settings.json:
```json
"Stop": [{"matcher": "*", "hooks": [{"type": "command", "command": "node .agent/hooks/notifications/notify.cjs"}]}]
```
Docs: `.agent/hooks/notifications/docs/`""",
    },
}
...
def discover_skills(skills_dir: Path) -> dict:
    """Scan .agent/skills/ and build skill catalog from SKILL.md files."""
    skills = {}
    categories = {}
...
def show_config_guide() -> None:
    ...
    print("- Global: `~/.agent/.ck.json` (user preferences)")
    print("- Local: `./.agent/.ck.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.agent/.ck.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.agent/.ck.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.agent/.ck.json - applies everywhere")
    ...
    print("# Local (.agent/.ck.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.agent/.ck.json` - personal preference")
    print("- Local: `./.agent/.ck.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    ...
    print("2. If 0-5, injects guidelines from `.agent/output-styles/coding-level-*.md`")
    ...
    print("## Customization")
    print()
    print("Guidelines live in `.agent/output-styles/coding-level-*.md`")
...
def main():
    # Find .agent/skills directory
    script_path = Path(__file__).resolve()
    # .agent/skills/ck-help/scripts/ck-help.py -> .agent/skills
