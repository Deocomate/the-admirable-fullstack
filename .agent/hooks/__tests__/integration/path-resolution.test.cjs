 ...
 * Run: node --test .agent/hooks/__tests__/integration/path-resolution.test.cjs
 *
...
describe('Issue #327: Path Resolution Integration', () => {
  ...
  describe('Subdirectory Workflow', () => {
    ...
    it('all hooks use CWD as base when in subdirectory', async () => {
      ...
      const subdirPath = path.join(gitRoot, '.agent', 'hooks');
      ...
      assert.ok(
        subagentContext.includes(subdirPath) || subagentContext.includes('.agent/hooks'),
  ...
  describe('Monorepo/Worktree Scenarios', () => {
    ...
    it('subagent uses payload.cwd for path resolution', async () => {
      ...
      const subdirPath = path.join(gitRoot, '.agent');
      ...
      assert.ok(
        context.includes(subdirPath) || context.includes('.agent'),
    ...
    it('scout-block uses payload.cwd to discover a git-root .agent/.ckignore', async () => {
      ...
      fs.mkdirSync(path.join(tempHome, '.agent'), { recursive: true });
      fs.writeFileSync(
        path.join(tempHome, '.agent', '.ck.json'),
      ...
      fs.writeFileSync(path.join(projectRoot, '.agent', '.ckignore'), '!build\n');
      ...
      try {
        ...
        assert.ok(
          blockResult.stderr.includes(path.join(projectRoot, '.agent', '.ckignore')),
