 ...
 * Run: node --test .agent/hooks/__tests__/session-init.test.cjs
 *
...
describe('session-init.cjs', () => {
  ...
  describe('Session Context Output', () => {
    ...
    it('loads previous session state during startup without session-state SessionStart hook', async () => {
      ...
      const stateDir = path.join(tempDir, '.agent', 'session-state');
    ...
    it('respects hooks.session-state=false during startup recovery', async () => {
      ...
      fs.mkdirSync(stateDir, { recursive: true });
      fs.writeFileSync(
        path.join(tempDir, '.agent', '.ck.json'),
    ...
    it('restores orphaned shadowed skills on startup', async () => {
      ...
      const shadowedSkillDir = path.join(tempDir, '.agent', 'skills', '.shadowed', 'cook');
      const restoredSkillDir = path.join(tempDir, '.agent', 'skills', 'cook');
      ...
      try {
        ...
        assert.ok(!fs.existsSync(path.join(tempDir, '.agent', 'skills', '.shadowed')), 'Startup should clean up the empty .shadowed directory');
  ...
  describe('Issue #291: Git Root Path Resolution', () => {
    ...
    it('shows subdirectory info when CWD differs from git root (Issue #327)', async () => {
      ...
      // Use .agent/hooks as subdirectory (guaranteed to exist)
      const subdirPath = require('path').join(gitRoot, '.agent', 'hooks');
