 ...
 * Run: node --test .agent/hooks/__tests__/team-context-inject.test.cjs
 *
 * Tests coverage:
...
function createTestTeam(baseDir, teamName) {
  const teamsDir = path.join(baseDir, '.agent', 'teams');
  const teamDir = path.join(teamsDir, teamName);
  const tasksDir = path.join(baseDir, '.agent', 'tasks');
...
describe('team-context-inject.cjs', () => {
  ...
  describe('Team Config Loading', () => {
    ...
    it('silently exits if team config does not exist', async () => {
      ...
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        // Set up .agent/teams directory but no team-a
        fs.mkdirSync(path.join(tmpDir, '.agent', 'teams'), { recursive: true });
    ...
    it('handles corrupted JSON config gracefully', async () => {
      ...
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        const teamsDir = path.join(tmpDir, '.agent', 'teams', 'team-a');
  ...
  describe('Peer List Building', () => {
    ...
    it('shows "none" as peers if agent is the only team member', async () => {
      ...
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        const teamDir = path.join(tmpDir, '.agent', 'teams', 'solo-team');
        fs.mkdirSync(teamDir, { recursive: true });
  ...
  describe('Task Summary Generation', () => {
    ...
    it('handles corrupted task files gracefully', async () => {
      ...
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        const taskDir = path.join(tmpDir, '.agent', 'tasks', 'team-a');
        fs.mkdirSync(taskDir, { recursive: true });
