...
```bash
cp .agent/hooks/notifications/.env.example ~/.agent/.env
```
...
Add to your `.agent/settings.json` or `.agent/settings.local.json`:
...
```bash
echo '{"hook_event_name":"Stop","cwd":"'$(pwd)'","session_id":"test123"}' | node .agent/hooks/notifications/notify.cjs
```

Expected output (if Telegram and Discord configured):
```
[env-loader] Loaded: /home/user/.agent/.env
...
- **Env Cascade**: `process.env` > `~/.agent/.env` > `.agent/.env`
...
**Blocked Patterns** (baseline in `.agent/.ckignore`, optional override in `<project-root>/.agent/.ckignore`):
- `node_modules`, `__pycache__`, `.git`, `dist`, `build`

**Testing:**
```bash
echo '{"tool_input":{"command":"ls node_modules"}}' | node .agent/hooks/scout-block.cjs
```
...
- Verify hooks are added to `.agent/settings.json`
