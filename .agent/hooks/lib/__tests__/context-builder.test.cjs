 ...
 * Run: node --test .agent/hooks/lib/__tests__/context-builder.test.cjs
 *
...
describe('context-builder.cjs', () => {
  ...
  describe('resolveRulesPath()', () => {
    ...
    it('returns null when file does not exist anywhere', () => {
      ...
      // Use a unique filename that won't exist in global ~/.agent/
    ...
    it('finds file in rules/ directory (new location)', () => {
      tempDir = createTempDir(['.agent/rules']);
      createTestFile(path.join(tempDir, '.agent/rules'), 'development-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('development-rules.md');
      assert.strictEqual(result, '.agent/rules/development-rules.md',
        'Should find file in rules/ directory');
    });

    it('falls back to workflows/ when rules/ does not exist', () => {
      tempDir = createTempDir(['.agent/workflows']);
      const uniqueFilename = `legacy-workflow-${Date.now()}-${Math.random().toString(36).slice(2)}.md`;
      createTestFile(path.join(tempDir, '.agent/workflows'), uniqueFilename);
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath(uniqueFilename);
      assert.strictEqual(result, `.agent/workflows/${uniqueFilename}`,
        'Should fall back to workflows/ directory');
    });

    it('prefers rules/ over workflows/ when both exist', () => {
      tempDir = createTempDir(['.agent/rules', '.agent/workflows']);
      createTestFile(path.join(tempDir, '.agent/rules'), 'development-rules.md', '# Rules version\n');
      createTestFile(path.join(tempDir, '.agent/workflows'), 'development-rules.md', '# Workflows version\n');
      process.chdir(tempDir);
    ...
    it('finds file in workflows/ when rules/ exists but file is only in workflows/', () => {
      ...
      createTestFile(path.join(tempDir, '.agent/workflows'), 'legacy-file.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('legacy-file.md');
      assert.strictEqual(result, '.agent/workflows/legacy-file.md',
  ...
  describe('resolveWorkflowPath alias', () => {
    ...
    it('resolveWorkflowPath works identically to resolveRulesPath', () => {
      const tempDir = createTempDir(['.agent/rules']);
      createTestFile(path.join(tempDir, '.agent/rules'), 'test.md');
  ...
  describe('Global path resolution', () => {
    ...
    it('checks global ~/.agent/rules/ path', () => {
      ...
      const tempDir = createTempDir(['.agent']);
      const originalCwd = process.cwd();
      process.chdir(tempDir);
      ...
      try {
        ...
        // Result depends on whether global ~/.agent/rules exists
  ...
  describe('buildReminderContext()', () => {
    ...
    it('includes devRulesPath when only workflows/ exists (backward compat)', () => {
      tempDir = createTempDir(['.agent/workflows']);
      createTestFile(path.join(tempDir, '.agent/workflows'), 'development-rules.md');
      process.chdir(tempDir);
  ...
  describe('Hooks config behavior (Issue #413)', () => {
    ...
    it('disables context section when context-tracking: false', () => {
      tempDir = createTempDir(['.agent']);
      const settingsPath = path.join(tempDir, '.agent', 'settings.json');
  ...
  describe('CLAUDE.md reference resolution', () => {
    ...
    it('resolves @rules/ references correctly', () => {
      ...
      tempDir = createTempDir(['.agent/rules']);
      createTestFile(path.join(tempDir, '.agent/rules'), 'primary-workflow.md');
      createTestFile(path.join(tempDir, '.agent/rules'), 'development-rules.md');
      createTestFile(path.join(tempDir, '.agent/rules'), 'orchestration-protocol.md');
      createTestFile(path.join(tempDir, '.agent/rules'), 'documentation-management.md');
      process.chdir(tempDir);
