...
function countConfigs(cwd) {
  ...
  const claudeDir = path.join(homeDir, '.agent');
  ...
  mcpCount += countMcpServersInFile(path.join(homeDir, '.agent.json'), userSettings);

  // Project scope
  if (cwd) {
    if (fs.existsSync(path.join(cwd, 'CLAUDE.md'))) claudeMdCount++;
    if (fs.existsSync(path.join(cwd, 'CLAUDE.local.md'))) claudeMdCount++;
    if (fs.existsSync(path.join(cwd, '.agent', 'CLAUDE.md'))) claudeMdCount++;
    if (fs.existsSync(path.join(cwd, '.agent', 'CLAUDE.local.md'))) claudeMdCount++;
    rulesCount += countRulesInDir(path.join(cwd, '.agent', 'rules'));
    mcpCount += countMcpServersInFile(path.join(cwd, '.mcp.json'));
    const projectSettings = path.join(cwd, '.agent', 'settings.json');
    mcpCount += countMcpServersInFile(projectSettings);
    hooksCount += countHooksInFile(projectSettings);
    const localSettings = path.join(cwd, '.agent', 'settings.local.json');
