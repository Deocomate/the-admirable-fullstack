 ...
 * @param {string} [configDirName='.agent'] - Config directory name
 * @returns {string|null} Resolved path or null
 */
function resolveRulesPath(filename, configDirName = '.agent') {
  // Try rules/ first (new location)
  const localRulesPath = path.join(process.cwd(), configDirName, 'rules', filename);
  const globalRulesPath = path.join(os.homedir(), '.agent', 'rules', filename);

  if (fs.existsSync(localRulesPath)) return `${configDirName}/rules/${filename}`;
  if (fs.existsSync(globalRulesPath)) return `~/.agent/rules/${filename}`;
  ...
  const globalWorkflowsPath = path.join(os.homedir(), '.agent', 'workflows', filename);

  if (fs.existsSync(localWorkflowsPath)) return `${configDirName}/workflows/${filename}`;
  if (fs.existsSync(globalWorkflowsPath)) return `~/.agent/workflows/${filename}`;
 ...
 * @returns {string|null} Resolved path or null
 */
function resolveScriptPath(filename, configDirName = '.agent') {
  const localPath = path.join(process.cwd(), configDirName, 'scripts', filename);
  const globalPath = path.join(os.homedir(), '.agent', 'scripts', filename);
  if (fs.existsSync(localPath)) return `${configDirName}/scripts/${filename}`;
  if (fs.existsSync(globalPath)) return `~/.agent/scripts/${filename}`;
  return null;
}
 ...
 */
function resolveSkillsVenv(configDirName = '.agent') {
  ...
  const globalVenv = path.join(os.homedir(), '.agent', 'skills', '.venv', venvBin, pythonExe);
  ...
  if (fs.existsSync(globalVenv)) {
    return isWindows
      ? '~\\.agent\\skills\\.venv\\Scripts\\python.exe'
      : '~/.agent/skills/.venv/bin/python3';
  }
  return null;
}
...
function buildRulesSection({ devRulesPath, skillsVenv, plansPath, docsPath }) {
  ...
  if (skillsVenv) {
    lines.push(`- Python scripts in .agent/skills/: Use \`${skillsVenv}\``);
  }
 ...
 * @param {string} [params.configDirName='.agent'] - Config directory name
 ...
 * }}
 */
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.agent', baseDir } = {}) {
