 ...
 * @param {string} [configDirName] - Config directory at git root (.agent, .opencode)
 * @returns {string|null}
 */
 ...
 * @param {string} [params.options.agentDir] - Path to .agent or .opencode directory
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  const {
    ckignorePath,
    projectCkignorePath,
    claudeDir = path.join(process.cwd(), '.agent'),
    cwd = process.cwd(),
