 ...
 * Storage: always global ~/.agent/session-states/{hash}/ to avoid project dir pollution
...
/** Resolve state dir: always global ~/.agent/session-states/{hash}/ to avoid project pollution */
function getStateDir(cwd) {
  try {
    const hash = crypto.createHash('md5').update(cwd).digest('hex').slice(0, 12);
    const globalDir = path.join(os.homedir(), '.agent', 'session-states', hash);
