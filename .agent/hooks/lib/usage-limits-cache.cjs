...
function readClaudeCredentials({
  ...
  try {
    const credentialsPath = path.join(homedir, '.agent', '.credentials.json');
...
function getClaudeAccessTokenFromCredentials(credentials) {
  return credentials?.agentAiOauth?.accessToken || null;
}

function hasSupportedClaudeSubscription(credentials) {
  const subscriptionType = String(credentials?.agentAiOauth?.subscriptionType || '').trim().toLowerCase();
  if (subscriptionType && subscriptionType !== 'free' && subscriptionType !== 'none') return true;

  const rateLimitTier = String(credentials?.agentAiOauth?.rateLimitTier || '').trim().toLowerCase();
