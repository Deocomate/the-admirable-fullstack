...
Add to `~/.agent/.env` (global) or `.agent/.env` (project):
...
Add to your `.agent/settings.json` (project) or `~/.agent/settings.json` (global):
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node .agent/hooks/notifications/notify.cjs"
          }
        ]
      }
    ]
...
echo '{"hook_event_name":"Stop","cwd":"'"$(pwd)"'","session_id":"test123"}' | \
  node .agent/hooks/notifications/notify.cjs
```
...
2. **.agent/.env** - Project-level Claude configuration
3. **.agent/hooks/.env** - Hook-specific configuration
...
#### Option C: `.agent/.env` (Project-Level Override)
...
#### Option D: `.agent/hooks/.env` (Hook-Specific)
...
Hooks are configured in `.agent/settings.local.json`:
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "node ${CLAUDE_PROJECT_DIR}/.agent/hooks/notifications/notify.cjs"
      }]
    }],
    "SubagentStop": [{
      "hooks": [{
...
1. **Verify `.agent/settings.local.json` exists and is valid JSON:**
   ```bash
   cat .agent/settings.local.json | node -e "process.stdin.resume();let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>console.log(JSON.parse(d)))"
   ```
...
**Implementation:** `.agent/hooks/notifications/notify.cjs` + `providers/telegram.cjs`

**Configuration:** `.agent/settings.local.json`
...
**Claude Code Hooks:** https://docs.agent.com/claude-code/hooks
