/**
 * Environment loader with cascade: process.env > ~/.agent/.env > .agent/.env
 ...
 * Priority: process.env > ~/.agent/.env > .agent/.env
...
function loadEnv(cwd = process.cwd()) {
  const envFiles = [
    ...
    path.join(cwd, '.agent', '.env'),
    path.join(os.homedir(), '.agent', '.env'),
  ];
