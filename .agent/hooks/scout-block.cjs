 ...
 *
 * Blocks access to directories listed in the shipped .agent/.ckignore baseline
 * plus an optional project-local .agent/.ckignore override at the git root.
 ...
 *
 * Configuration:
 * - Edit .agent/.ckignore to customize shipped baseline patterns
 * - Add <project-root>/.agent/.ckignore to override locally without changing the baseline
...
try {
  ...
  try {
    ...
    const claudeDir = path.dirname(__dirname); // Go up from hooks/ to .agent/
