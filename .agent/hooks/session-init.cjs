...
function cleanupOrphanedShadowedSkills() {
  const shadowedDir = path.join(process.cwd(), '.agent', 'skills', '.shadowed');
 ...
 * Scans ~/.agent/teams/ for active team configs and checks membership.
...
function detectAgentTeam() {
  try {
    const teamsDir = path.join(os.homedir(), '.agent', 'teams');
...
async function main() {
  ...
  try {
    ...
    if (envFile) {
      ...
      writeEnv(envFile, 'CK_CLAUDE_SETTINGS_DIR', staticEnv.agentSettingsDir);
    ...
    if (hasCleanup) {
      ...
      if (shadowedCleanup.kept.length > 0) {
        ...
        console.log(`    Review .agent/skills/.shadowed/ and merge changes manually.`);
      }
    }
    ...
    if (teamInfo) {
      ...
      console.log(`    Team config: ~/.agent/teams/${teamInfo.teamName}/config.json`);
