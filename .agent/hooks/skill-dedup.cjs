 ...
 *
 * Original design:
 *   SessionStart: move overlapping local skills to .agent/skills/.shadowed/
 *   SessionEnd:   restore .shadowed/ skills back to .agent/skills/
 *
...
function getDefaultPaths() {
  const globalDir = path.join(os.homedir(), '.agent', 'skills');
  const localDir = path.join(process.cwd(), '.agent', 'skills');
