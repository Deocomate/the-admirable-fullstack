 ...
 *
 * Run: node --test .agent/hooks/tests/scout-block/scout-checker.test.cjs
 */
...
function createProjectRoot({ rootPatterns, nestedPatterns } = {}) {
  ...
  if (rootPatterns) {
    ...
    fs.writeFileSync(path.join(repoRoot, '.agent', '.ckignore'), `${rootPatterns.join('\n')}\n`);
  }
  if (nestedPatterns) {
    fs.mkdirSync(path.join(repoRoot, 'pkg', '.agent'), { recursive: true });
    fs.writeFileSync(path.join(repoRoot, 'pkg', '.agent', '.ckignore'), `${nestedPatterns.join('\n')}\n`);
  }
  return repoRoot;
}
...
describe('isVenvExecutable', () => {
  ...
  it('matches nested .venv path', () => {
    assert.ok(isVenvExecutable('~/.agent/skills/.venv/bin/python3 script.py'));
  });
...
describe('checkScoutBlock - Bash commands', () => {
  ...
  describe('allowed venv commands', () => {
    it('allows .venv/bin/python3 execution', () => {
      const r = checkScoutBlock({ toolName: 'Bash', toolInput: { command: '~/.agent/skills/.venv/bin/python3 script.py' }, options: DEFAULT_OPTS });
      assert.ok(!r.blocked);
    });
...
describe('checkScoutBlock - project-local .agent/.ckignore discovery', () => {
  it('allows build paths via the git-root .agent/.ckignore override', () => {
    ...
    const r = checkScoutBlock({
      ...
      options: { ...DEFAULT_OPTS, cwd: projectRoot, projectConfigDirName: '.agent' }
    });
    assert.ok(!r.blocked);
  });

  it('still blocks node_modules and points to the git-root .agent/.ckignore file', () => {
    const projectRoot = createProjectRoot({ rootPatterns: ['!build'] });
    const r = checkScoutBlock({
      toolName: 'Read',
    ...
    assert.ok(r.blocked);
    assert.strictEqual(r.configPath, path.join(projectRoot, '.agent', '.ckignore'));
  });
  ...
  it('ignores nested .agent/.ckignore files below the git root', () => {
    ...
    const r = checkScoutBlock({
      toolName: 'Read',
      toolInput: { file_path: 'src/commands/build/run.rb' },
      options: { ...DEFAULT_OPTS, cwd: nestedCwd, projectConfigDirName: '.agent' }
    });
    assert.ok(r.blocked);
  });
});
