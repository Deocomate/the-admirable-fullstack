...
pip install -r .agent/skills/ai-multimodal/scripts/requirements.txt
...
```bash
cd .agent/skills/ai-multimodal/scripts
...
**ai-multimodal** (`.agent/skills/ai-multimodal/scripts/requirements.txt`):
...
```bash
cd .agent/skills/{skill-name}/scripts
...
2. **`.agent/skills/{skill-name}/.env`** (skill-specific config)
3. **`.agent/skills/.env`** (shared skills config)
4. **`.agent/.env`** (global Claude config)
...
# Run all tests
pytest .agent/skills/*/scripts/tests/ -v

# Check coverage across all skills
pytest .agent/skills/*/scripts/tests/ --cov=.agent/skills --cov-report=html
```
