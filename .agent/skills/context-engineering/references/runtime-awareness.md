...
| Windows | File | `%USERPROFILE%\.agent\.credentials.json` |
| Linux | File | `~/.agent/.credentials.json` |
...
### Hook Settings (`.agent/settings.json`)
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node .agent/hooks/usage-quota-cache-refresh.cjs"
        }]
      }
    ]
  }
