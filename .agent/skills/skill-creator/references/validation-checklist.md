...
- [ ] Env vars: respects hierarchy `process.env` > `$HOME/.agent/skills/${SKILL}/.env` (global) > `$HOME/.agent/skills/.env` (global) > `$HOME/.agent/.env` (global) > `./.agent/skills/${SKILL}/.env` (cwd) > `./.agent/skills/.env` (cwd) > `./.agent/.env` (cwd)
