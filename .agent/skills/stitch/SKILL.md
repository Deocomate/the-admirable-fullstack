...
Add `STITCH_API_KEY=sk_...` to `~/.agent/.env` (or `~/.agent/skills/.env`).
...
```bash
cd ~/.agent/skills/stitch/scripts && npm install
```

Or run `~/.agent/skills/install.sh` which handles this automatically.
...
Add to `~/.agent/.mcp.json` for native design context in Claude Code:
...
- Local tracking via `~/.agentkit/.stitch-quota.json`
