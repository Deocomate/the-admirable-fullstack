...
Teammates with `memory: project` in their agent definition retain learnings across team sessions. Memory persists in `.agent/agent-memory/<name>/` (gitignored). Useful for:
...
Memory persists after team cleanup -- it's in `.agent/agent-memory/`, not `~/.agent/teams/`.
...
If unresponsive: close terminal or kill session. Clean orphaned configs at `~/.agent/teams/` manually.
...
See `.agent/rules/team-coordination-rules.md` for teammate behavior rules.
