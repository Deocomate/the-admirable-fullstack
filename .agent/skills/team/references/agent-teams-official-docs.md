...
> **Canonical source:** https://code.agent.com/docs/en/agent-teams
...
| **Task list** | Shared work items at `~/.agent/tasks/{team-name}/` |
| **Mailbox** | Messaging system for inter-agent communication |

Storage:
- **Team config**: `~/.agent/teams/{team-name}/config.json` (members array with name, agent ID, type)
- **Task list**: `~/.agent/tasks/{team-name}/`
...
**Custom subagents:** Define in `.agent/agents/` with frontmatter (name, description, tools, model).
...
|-------|----------|-----------------|
| `user` | `~/.agent/agent-memory/<name>/` | All projects |
| `project` | `.agent/agent-memory/<name>/` | Sessions in same project |
