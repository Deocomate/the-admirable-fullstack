...
def test_urlencoded_method_override_to_put() -> None:
    client = TestClient(_build_app())
    response = client.post("/echo", data={"_method": "PUT", "name": "Marie Curie"})
...
def test_urlencoded_method_override_to_delete() -> None:
    client = TestClient(_build_app())
    response = client.post("/echo", data={"_method": "DELETE"})
    assert response.json()["method"] == "DELETE"


def test_multipart_with_file_upload_survives_method_override() -> None:
...
def test_no_method_override_field_keeps_post() -> None:
    client = TestClient(_build_app())
    response = client.post("/echo", data={"name": "Marie Curie"})
    assert response.json()["method"] == "POST"


def test_unsupported_override_value_keeps_post() -> None:
    client = TestClient(_build_app())
    response = client.post("/echo", data={"_method": "PATCHXYZ", "name": "x"})
    assert response.json()["method"] == "POST"
